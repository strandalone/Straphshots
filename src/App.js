// Straphshots - Fully Responsive and Optimized
import { useState, useEffect } from "react";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";

export default function Straphshots() {
  ...
  const addShots = (name) => {
    ...
    if (!isNaN(amount)) {
      setShotsTaken({ ...shotsTaken, [name]: (shotsTaken[name] || 0) + amount });
  ...
  const mvp = people.reduce((best, name) => {
    const total = getTotalPoints(name);
    return total > best.points ? { name, points: total } : best;
  }, { name: "—", points: -Infinity });
    ...
    <div className="relative min-h-screen bg-green-950 text-white p-4 space-y-8 overflow-hidden">
      ...
      <div className="flex flex-row justify-center items-end gap-4 sm:gap-6 flex-wrap">
        {[1, 0, 2].map((pos, i) => {
          ...
          return (
            <div key={i} className={`flex flex-col justify-end items-center text-center rounded-xl ${
              pos === 0 ? 'bg-green-600 h-36 w-24 sm:h-56 sm:w-1/4' :
              pos === 1 ? 'bg-green-700 h-32 w-20 sm:h-44 sm:w-1/5' :
      ...
      <div className="space-y-2 bg-green-800 p-4 rounded-xl">
        <h2 className="text-xl font-bold text-green-300 mb-2 text-center">Leaderboard</h2>
        <ul className="space-y-1">
          {[...people]
            .sort((a, b) => getTotalPoints(b) - getTotalPoints(a))
            .map((name, index) => (
              <li key={name} className="flex justify-between border-b border-green-600 pb-1">
                <span>{index + 1}. {name}</span>
                <span className="text-green-300 font-mono">{getTotalPoints(name)} pts</span>
              </li>
          ))}
        </ul>
      </div>
      ...
      <div className="bg-white text-[#0d1f14] p-4 rounded-xl">
        <strong>Most Points Given:</strong> {topReason.name} got "{topReason.reason}" worth {topReason.points} points!
      </div>
            ...
            <div className="flex justify-between items-center mb-2">
              {admin ? (
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const updatedPeople = people.map((p) => p === name ? newName : p);
                    const updatedReasons = { ...reasons, [newName]: reasons[name] || [] };
                    const updatedShots = { ...shotsTaken, [newName]: shotsTaken[name] || 0 };
                    const updatedInputs = { ...inputStates, [newName]: inputStates[name] || { reason: "", points: "", shots: "" } };
              ...
              {admin && (
                <button onClick={() => {
                  ...
                  setReasons(r); setShotsTaken(s); setInputStates(i);
